...
// @FIXME(alekum) add more detailed tracing because the valude of g_shadow_pages is SymExpr*!
// we can try dump SymExpr with deps here to see how actually we getting and handling our dependencies
#if DEBUG_RUNTIME
[[maybe_unused]]
void dump_known_regions() {
  std::cerr << "Known regions:" << std::endl;
  for (const auto &[page, shadow] : g_shadow_pages) {
    std::cerr << "  " << reinterpret_cast<void*>(page) << " shadowed by " << reinterpret_cast<void*>(shadow) << std::endl;
  }
}
#endif
...
#if DEBUG_RUNTIME
  std::cerr << "Reading " << length << " bytes from address " << reinterpret_cast<void*>(addr)
            << std::endl;
  dump_known_regions();
#endif
...
#if DEBUG_RUNTIME
  std::cerr << "Writing " << length << " bytes to address " << reinterpret_cast<void*>(addr)
            << std::endl;
  dump_known_regions();
#endif
