...
Expr::Expr(Kind kind, UINT32 bits)
  // : DependencyNode() // @Information(alekum): we get rid of DependencyNode class completely
  // , 
  : kind_(kind)
  , bits_(bits)
  , children_()
...
#if DEBUG_RUNTIME
  std::cerr << "Compute Deps for [ "  << this << "::" << this->toString() << " ]\n";
#endif
  DependencySet deps;
...
/// Return vector of uses for this expression
/// @return const ref to uses_ vector
const WeakExprRefVectorTy& Expr::uses() const {
  return this->uses_;
}

/// Sometimes we need manually force uses clearing...
void Expr::clearUses() {
  this->uses_.clear();
}
...
// @Information(alekum): should ease the printing traces
ostream& operator<<(ostream& os, const Expr& e) {
  e.print(os);
}

void Expr::print(ostream& os, UINT depth) const {
  os << getName() << "[" << this << ", isConcrete=" << (isConcrete_ ? "true" : "false") << "]::(";
  bool begin = !printAux(os);
  printChildren(os, begin, depth);
  os << ")";
}

// @TODO(alekum): this part actually prints just range set...
