...
class ExprBuilder;
// @Cleanup(alekum) : could not even find what is it and where it is being used
// class IndexSearchTree;

//typedef std::set<INT32> DepSet;
using DepSet = std::set<size_t>;
...
class Expr { //}: public DependencyNode {
  public:
    ...
    inline void setBitness(UINT32 bits) { bits_ = bits; }
    ...
    /// @Cleanup(alekum) We need update children_ vector
    /// Current impl doesn't provide other options to deal with
    /// vector of childrens, so let's keep it like this
    /// FIXME
    inline  void SwapChild(ExprRef e, UINT32 index) {
      children_[index] = e;
      invalidate();
    }
    ...
    // Just track concrete deps
    inline DepSet& getCDeps() {
      if (cdeps_ == nullptr) cdeps_ = new DepSet();
      return *cdeps_;
    }

    // @Information(alekum) As we removed DependencyNode class entirely this non-sense is over
    inline DependencySet computeDependencies(); //override;
    ...
    z3::expr& toZ3Expr(bool verbose=false) {
      // @FIXME(alekum) right now we have to remove such caching...
      // we should invalidate model here...up to parent_root if we invalidated children
      // otherwise right now comment this check
      //if (expr_ == NULL) {
        delete expr_; // remove this if above check will be returned
...
#if DEBUG_RUNTIME
      std::cerr << "[Expr::addChild]  this = " << this
                << ", addChild = [ " << e->toString() << " ]\n";
#endif
    ...
    /// Set isConcrete_ = false for users, will affect z3 expr generation
    void symbolize() {
      if(isConcrete_) {
        isConcrete_ = false;
        // @Cleanup(alekum): generalize this part....whis concretization...
        for (auto it = uses_.begin(); it != uses_.end(); it++) {
          WeakExprRef& ref = *it;
          if (ref.expired())
            continue;
          ref.lock()->trySymbolize();
        }
      }
    }

    /// Set isConcrete_ = false for children, will affect z3 expr generation
    void trySymbolize() {
      if (!isConcrete())
        return;
      // @Cleanup(alekum): generalize this part....whis concretization...
    ...
    /// Set isConcrete_ = true for users, will affect z3 expr generation
    void concretize() {
      if (!isConcrete()) {
        isConcrete_ = true;
        for (auto it = uses_.begin(); it != uses_.end(); it++) {
    ...
    /// Set isConcrete_ = true, for children, will affect z3 expr generation
    void tryConcretize() {
      if (isConcrete())
        return;
    ...
    /// Returns vector of uses for current Expr
    const WeakExprRefVectorTy& uses() const;
    void clearUses();
    
    // @Information(alekum)
    // if we invalidate expression then we have to mark it as invalidated
    // and internally when we wanna create z3::expr we should check also 
    // whether we invalidated or not our expression
    // condition of invalidation for now:
    //    swapChilren -> it invalidates our z3::ast(we don't care about children)
    //    clearUses -> this invalidates parent's z3::ast ( parents problem )
    // 
    // Another strategy - try substitute, but for that we have to expose z3::expr* internals...
    // should we invalidate range_sets???
    inline void invalidate() { 
      delete expr_;         expr_       = nullptr;
      delete hash_;         hash_       = nullptr;
      delete deps_;         deps_       = nullptr;
      evaluation_.reset();  evaluation_ = nullptr;
      depth_ = -1;
      // invalidate all users who uses us also...
      // otherwise cached values might messing up with following computations
      for(const auto& wrUse : uses_) {
        if(!wrUse.expired()) {
          wrUse.lock()->invalidate();
        }
      }
    }
  ...
  protected:
    ...
    INT32 depth_;
    DepSet* deps_; // Probably the better idea to keep pair of dep -> ExprRef
    DepSet* cdeps_; // Let's keep track of concrete dependencies
    WeakExprRefVectorTy uses_; // this is actually list of users!!
...
protected:
  bool printAux(ostream& os) const override {
    os << "index=" << hexstr(index_) << ", bits=" << bits_;
    return true;
  }

  z3::expr toZ3ExprRecursively(bool verbose) override {
    if(isConcrete_) {
      // @Information(alekum): It should Return ConstantExpr
      // if we started to be concrete, let us be concrete....
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    ...
    // @Cleanup(alekum) we assume that currently child is ReadExpr...so....
    if(e->isConcrete()){
      isConcrete_ = true;
      return evaluate()->toZ3Expr(verbose);
    }
