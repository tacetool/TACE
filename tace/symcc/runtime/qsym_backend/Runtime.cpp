...
void dump_allocated_expressions_stats(std::ostream &os=std::cerr) {
  os << "===================== ALLOCATED EXPRESSIONS STATS : " 
      << allocatedExpressions.size() << " =====================\n";
  size_t idx = 0;
  for(const auto& [k,v] : allocatedExpressions) {
    os << "IDX=" 
        << idx++ 
        << " :: " 
        << k->toString() 
        ...
        << k->getDeps().size()
        << " [ ";
        for(const auto d : k->getDeps()) os << d << ' ';
    os << "]"
        << '\n';
  }
  os << "===================== ALLOCATED EXPRESSIONS STATS =====================\n\n";
...
#if DEBUG_RUNTIME
  std::cerr << "[registerExpression] :: " << rawExpr->toString() << std::endl;
#endif
...
void _sym_push_path_constraint(SymExpr constraint, int taken,
  ...
  if (constraint == nullptr)
    return;
#if DEBUG_RUNTIME
  std::cerr << "\nPush Path Constrain " << constraint << " : {\n\t" << constraint->toString() << "\n}\n" << std::endl;
#endif
  g_solver->addJcc(allocatedExpressions.at(constraint), taken != 0, site_id);

#if DEBUG_RUNTIME
  dump_allocated_expressions_stats();
#endif
}

SymExpr _sym_get_input_byte(size_t offset) {
  auto se = registerExpression(g_expr_builder->createRead(offset));
#if DEBUG_RUNTIME
  LOG_INFO("\nGet Read SymExpr from input by offset "+ std::to_string(offset) +": {\n\t" + se->toString() + "\n}\n");
#endif
  return se;
}
